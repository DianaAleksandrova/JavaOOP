...
public class GangNeighbourhood implements Neighbourhood{
    ...
    public void action(Player mainPlayer, Collection<Player> civilPlayers) {
        ...
        for (Player civilPlayer : civilPlayers) {
            ...
            Deque<Gun> gunsPlayers = new ArrayDeque<>(civilPlayer.getGunRepository().getModels());
                Gun gunPlayers = gunsPlayers.poll();
                while (gunPlayers != null) {
                    while (gunPlayers.canFire() && mainPlayer.isAlive()) {
                        int fire = gunPlayers.fire();
                        mainPlayer.takeLifePoints(fire);
                    }
                    if (!mainPlayer.isAlive()) {
                        return;
                    }
                    gunPlayers = gunsPlayers.poll();
                }
            }
        }
    }
